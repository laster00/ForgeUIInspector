...
def grid_page_size(grid: Any, label: str) -> int:
    if not isinstance(grid, dict):
        fail(f"{label} grid must be an object")
    columns = grid.get("columns")
    rows = grid.get("rows")
    slots = grid.get("slots")
    if not is_integer(columns) or not is_integer(rows) or columns <= 0 or rows <= 0:
        fail(f"{label} grid columns/rows must be positive integers")
    if not is_integer(slots) or slots != columns * rows:
        fail(f"{label} grid slots must equal columns * rows")
    return slots


def item(index: int, page_size: int, layout: str = "all") -> dict[str, Any]:
...
def make_fixture(fixture_id: str, count: int, page_size: int) -> dict[str, Any]:
    layout = "other" if fixture_id == "other" else "all"
    items = [item(index, page_size, layout) for index in range(count)]
    return {
        "id": fixture_id,
...
def generate(args: argparse.Namespace) -> int:
    ...
    grid = {"columns": args.columns, "rows": args.rows, "slots": args.columns * args.rows}
    page_size = grid_page_size(grid, f"screen {screen_id}")
    project = {
    ...
    fixture = {
        ...
        "pageSize": page_size,
        "fixtures": [
            make_fixture("normal", min(18, page_size), page_size),
            make_fixture("empty", 0, page_size),
            make_fixture("many", page_size + 1, page_size),
            make_fixture("other", min(6, page_size), page_size),
        ],
    }
...
def validate_fixture(data: dict[str, Any], path: Path, project_id: str, screen_id: str, page_size: int) -> None:
    if data.get("schema") != FIXTURE_SCHEMA or not is_integer(data.get("version")) or data.get("version") != 1:
    ...
    if not is_integer(data.get("pageSize")) or data.get("pageSize") != page_size:
    ...
    for fixture in fixtures:
        ...
        for layout in layouts:
            ...
            if not is_integer(layout.get("count", 0)) or layout.get("count", 0) < 0:
        ...
        for index, entry in enumerate(items):
            if not isinstance(entry, dict) or not is_integer(entry.get("slot")) or entry.get("slot") != index % page_size:
                fail(f"item {index} slot must be {index % page_size}: {path}")
            if "page" in entry and (not is_integer(entry.get("page")) or entry.get("page") != index // page_size):
                fail(f"item {index} page must be {index // page_size}: {path}")
            if "count" in entry and (not isinstance(entry["count"], (int, float)) or entry["count"] < 0):
                fail(f"item count must be non-negative: {path}")
        item_count = fixture.get("itemCount", 0)
        if not is_integer(item_count) or item_count < 0:
            fail(f"itemCount must be non-negative: {path}")
        if items and "itemCount" in fixture and item_count != len(items):
            fail(f"fixture {fixture_id} itemCount must equal items length {len(items)}: {path}")
        effective_counts[fixture_id] = len(items) if items else item_count
    for required in ("normal", "empty", "many", "other"):
        if required not in fixture_ids:
            fail(f"missing required fixture {required}: {path}")
    if effective_counts["many"] <= page_size:
        fail(f"many fixture must contain more than {page_size} items: {path}")
...
def validate_project(path: Path) -> int:
    ...
    if manifest.get("schema") != PROJECT_SCHEMA or not is_integer(manifest.get("version")) or manifest.get("version") != 1:
...
def parser() -> argparse.ArgumentParser:
    ...
    init.add_argument("--columns", type=int, default=DEFAULT_COLUMNS)
    init.add_argument("--rows", type=int, default=DEFAULT_ROWS)
